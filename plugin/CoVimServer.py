...
parser.add_argument('port', type=int, nargs='?', default=25000,
...
class React(Protocol):
  ...
  def dataReceived(self, data):
    if self.state == "GETNAME":
      ...
      # Convert to a json string instead of bytes so that we can store data inside a string
      if isinstance(data, bytes):
        data_string = data.decode('utf-8')
      self.buffer += data_string
      # If we find a valid JSON object
      if self.handle_BUFF(self.buffer):
  ...
  def connectionLost(self, reason):
    if hasattr(self, 'user'):
      ...
      # I want this to run all the time so that I can connect to it at any time
